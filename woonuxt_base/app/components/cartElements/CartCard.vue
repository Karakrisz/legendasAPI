...
const customPrice = computed(() => {
  const customPriceData = item.extraData?.find(data => data.key === 'total_custom_price');
  console.log(customPriceData);
  return customPriceData ? parseFloat(customPriceData.value) : null;
});
</script>
  ...
  <SwipeCard @remove="removeItem">
    <div v-if="productType" class="flex items-center gap-3 group">
      ...
      <div class="flex-1">
        ...
        <ProductPrice class="text-xs" :sale-price="productType.salePrice" :regular-price="customPrice ? `${customPrice} Ft` : productType.regularPrice" />
      </div>
